 ...
 * @see #dispose()
 */
public class ROS2ClientV2 extends RoboticsAPIApplication {
	@Inject
...
//	private JointImpedanceControlMode jointImpedanceControl;
	...
	ArrayList<double[]> joint_observation_data = new ArrayList<double[]>();
...
//		    			TCP_IP_Handler.getStringRequest(); //Handshake
		    		...
		    		double[] current_joint_observation = new double[7];
		    		current_joint_observation = lBR_iiwa_14_R820_1.getCurrentJointPosition().get(); 
		    		joint_observation_data.add(current_joint_observation);					
		    	}
			}
...
//				ReadWriteUtilities.display2DListDouble(observation_data);
			...
			else if(command_msg.equals("recv_joint_obs")){
				System.out.println("Sending observations");
//				ReadWriteUtilities.display2DListDouble(observation_data);
				TCP_IP_Handler.sendNPArrResponse(joint_observation_data);
				TCP_IP_Handler.sendStringResponse("Done"+"\r\n");
