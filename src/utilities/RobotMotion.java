...
public class RobotMotion {
	...
	/**
	 * Class constructor
	 * @param lBR_iiwa_7_R800_1
	 * @param tcp
	 */
	public RobotMotion(LBR lBR_iiwa_7_R800_1, ObjectFrame tcp)
	{
		...
		if (N != 0)
		{
			...
			for (double [] joint_state : joint_config) 
			{
				try
				{
					...
					System.out.println("idx : " + idx_val++);
					
//					Timedelay.wait_milliseconds(500);
	...
	*/
	...
	public void approachTrajectory(ArrayList<double[]> joint_config)
	{
		...
		Frame F = lBR_iiwa_7_R800_1.getForwardKinematic(start_joint_state);
		System.out.println("Approaching the trajectory start point...");
		try
		{
			F.setZ(F.getZ() + approachDistance);
			lBR_iiwa_7_R800_1.getFlange().move(ptp(F).setJointVelocityRel(approachTrajectorySpeed).setMode(impedanceMode));
		}
		catch (Exception e)
		{
	...
	public void exitTrajectory(Frame F)
	{
		...
		while (i<5)
		{
			try
			{
				...
				mtn = lBR_iiwa_7_R800_1.getFlange().moveAsync(ptp(F).setJointVelocityRel(retractTrajectorySpeed).setMode(retractImpedanceMode));
				i++;
			}
			catch (Exception e)
			{
