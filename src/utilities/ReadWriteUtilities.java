...
public class ReadWriteUtilities {
	...
	public static void display2DListDouble(double[] o)
	{
		if (o.length!=0)
		{	String S = "";
	...
	/**
	 * Writes double ArrayList to the file. </br> 
	 * @param obj
	 */
	public void writeDoubleArrayListToFile(ArrayList <double[]> data)
	{
		try
		{
			for (int i=0;i<data.size();++i)
			{
				...
				{
					s = s + Double.toString(data.get(i)[j]);
					if (j!=data.get(i).length-1) s = s + ",";
				}
//				System.out.println(s);
				bw.write(s+ "\n");		
			}
	...
	/**
	 * Writes int ArrayList to the file. </br> 
	 * @param obj
	 */
	public void writeIntArrayListToFile(ArrayList <int[]> data)
	{
		try
		{
			for (int i=0;i<data.size();++i)
			{
				...
				for (int j=0;j<data.get(i).length;++j)
				{
					s = s + Integer.toString(data.get(i)[j]);
					if (j!=data.get(i).length-1) s = s + ",";
				}
				System.out.println(s);
				bw.write(s+ "\n");		
	...
	public static void display2DList_Double(ArrayList<Double> o) 
	{
		if (o.isEmpty()==false)
		{
		for (int i=0; i<o.size(); ++i)
