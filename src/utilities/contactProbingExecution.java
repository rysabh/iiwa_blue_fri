...
public class contactProbingExecution {
	...
	public contactProbingExecution(LBR rob_in, ObjectFrame tool_tip, ArrayList<double[]> joint_config, ArrayList<double[]> cartesian_config)
	{
		...
		probeImpedence.parametrize(CartDOF.Z).setStiffness(100);	
	...
	public void approachSurfaceForProbing(ArrayList<double[]> joint_config_partial)
	{
		...
		try
		{
			...
			while(true && approach_dist_reducer<probing_approach_dist)
			{
				try
				{
					F_st = lBR_iiwa_7_R800_1.getForwardKinematic(start_joint_state);
					F_st.setZ(F_st.getZ()+probing_approach_dist-approach_dist_reducer); 
					lBR_iiwa_7_R800_1.getFlange().move(ptp(F_st).setJointVelocityRel(0.3).setMode(approachImpedence));
					break;
				}
				catch(Exception e)
				{
			...
			F_st = lBR_iiwa_7_R800_1.getCurrentCartesianPosition(tool_tip);
			boolean flag = true;
			while(flag)
			{
				...
				{
					ForceSensorData flangeforce = lBR_iiwa_7_R800_1.getExternalForceTorque(tool_tip);
	...
	*/
	...
	{
	// using joint config
	RobotMotion probeMotion = new RobotMotion(lBR_iiwa_7_R800_1, tool_tip);
	probeMotion.setSpeed(0.03);
...
//				System.out.println(Fz);
		        Frame store_frame = lBR_iiwa_7_R800_1.getCurrentCartesianPosition(lBR_iiwa_7_R800_1.getFlange());
			...
			{
				MyMotion.cancel();
				Frame F_st = lBR_iiwa_7_R800_1.getCurrentCartesianPosition(tool_tip);
				try{
	...
	{	
		...
		for (int i=0; i<grp_idx.size(); ++i)
		{
			...
			exeObj = new contactProbingExecution(lBR_iiwa_7_R800_1, tool_tip, joint_config_partial, cartesian_config_partial);
