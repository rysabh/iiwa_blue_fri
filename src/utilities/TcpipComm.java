...
//			System.out.println(data);
	...
	public double[] getDoubleArrayRequest()
	{
		...
		{	
			...
			if(data_arr.length == 1){
				System.out.println("In data func: "+data);
			}
		...
		for (int i=0;i<arr.length;++i){
			arr[i] = Double.valueOf(data_arr[i]);
		}
	...
	public ArrayList<double[]> getDoubleNPArrRequest()
	{	sendStringResponse("s\r\n");
		String val = getStringRequest();
		int data_size = Integer.valueOf(val);
...
//		int data_size = Integer.valueOf(val);
//		
//		System.out.println("Data Size: "+String.valueOf(data_size));
//		ArrayList <double[]> data_mat = new ArrayList<double[]>(data_size);
//		//New code for receiving Numpy arrays
//		String data;			
//		
//		try
//		{	
...
//				double[] row = new double[data_arr.length];
//				for(int i = 0; i<data_arr.length;i++){
//					row[i] = Double.parseDouble(data_arr[i]);
//				}
//				data_mat.add(row);
//			}
//		
...
//	}
	...
	public double[] getDoubleNP1DArrayRequest()
	{
		String [] data_arr = null;
		try
		{	
	...
	public void sendNP1DArrResponse(double[] resp_arr)
	{
		sendStringResponse(String.valueOf(resp_arr.length)+"\r\n");
		getStringRequest();
		for (int i=0;i<resp_arr.length;++i)
		{
			sendStringResponse(String.valueOf(resp_arr[i]));
			getStringRequest();
		}
	}
	
	public void sendNPArrResponse(ArrayList<double[]> resp_mat)
	{	
		//Sending Num Rows
		sendStringResponse(String.valueOf(resp_mat.size())+"\r\n");
		getStringRequest();
		//Sending Num Cols
		sendStringResponse(String.valueOf(resp_mat.get(0).length)+"\r\n");
		getStringRequest();
		DataOutputStream resp = null;
		try {
			resp = new DataOutputStream(socket.getOutputStream());
		...
		// Send the ArrayList
        write_data.flush();
        for(double [] mat: resp_mat){
			for(int i = 0; i<mat.length;i++){				
				write_data.println(mat[i]);				
			}
		}
		write_data.println("eof");
		write_data.flush();
	 ...
	 * </code>
	 */
	public void sendLongResponse(long resp_num)
	{
		try
		{
			DataOutputStream resp = new DataOutputStream(socket.getOutputStream());
	...
	public void sendLongArrayResponse(long[] resp_arr)
	{
		sendLongResponse(resp_arr.length);
		getStringRequest();
		for (int i=0;i<resp_arr.length;++i)
		{
			sendLongResponse(resp_arr[i]);
			getStringRequest();
		}
	}
	...
	public void sendLongMatrixResponse(ArrayList<long[]> resp_mat)
	{
		sendIntResponse(resp_mat.size());
		getStringRequest();
		sendIntResponse(resp_mat.get(0).length);
		...
		for (int i=0;i<resp_mat.size();++i)
		{
			sendLongArrayResponse(resp_mat.get(i));
		}
	}
